...
本次 web 前端规范 V1.0 主要包括：html 规范，css 规范，javascript 规范，vue 规范。
...
- 项目/文件夹：小写，使用中划线连接。如`management-system`
- JS、CSS、SCSS、HTML、图片文件命名：小写，中划线连接。如`main-layout.css`
- js 文件：当输出为 Class 或者构造函数时，首字母使用大写。文件名使用驼峰命名
- 代码中的命名严禁使用拼音与英文混合的方式，更不允许直接使用中文的方式。 正确的英文拼写和语法可以让阅读者易于理解，避免歧义。注意，即使纯拼音命名方式也要避免采用。
  `rmb` 等国际通用的名称，可视同英文。
- 杜绝完全不规范的缩写，避免望文不知义。如 condition 缩写命名成 condi。此类随意缩写严重降低了代码的可阅读性。

## HTML 规范

1. 文档类型  
   一个 HTML 文件必须指定文档标准，这样浏览器才会按照指定的标准去解释 HTML 文档。目前最常用的文档标准为 HTML5 标准。
   `<!DOCTYPE html>` 将这句写进 HTML 文件的第一行。
2. 编码  
   使用 UTF-8 作为文档的编码标准 `<meta charset="utf-8">`
3. 缩进  
   为每个块级元素新起一行，并且对每个子元素进行缩进，每次缩进 `4` 个空格，不使用 tab 来缩进。
4. 标签
...
- 合理的使用 HTML 注释，如非必要，不要添加 HTML 注释

```html
<!-- 注释内容，前后预留一个空白 -->
```

- 当有个工作稍后再做时，可以添加 todo 注释
...
9. 指令都使用缩写形式 10.单文件组件应该总是让标签顺序保持为
